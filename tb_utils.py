...
class BBoxUtility(object):
    ...
    def multiScaleOutput(self, results):
        """Combining multiple scales of output together.

        # Arguments
            results: A list have length of scales used for testing. 
            Every entry contain the list of bounding boxes in result for 
            all pictures. Also applying NMS among all scales for single
            picture.

        # Return
            preds: A list conatains combined bounding boxes for all pictures.
        """
        ...
        preds = [np.concatenate(x) for x in results]
        result = []
        for i in range(len(preds)):
            ...
            feed_dict = {self.boxes: loc,
                         self.scores: conf}
            idx = self.sess.run(self.nms, feed_dict=feed_dict)
            result.append(pred[idx])
        return result
    def combineMultiScaleOutput(self, results):
        results = np.transpose(np.asarray(results),(1,0))
        preds = [np.concatenate(x) for x in results]
        return preds

def loadWeightsFromNumpy(model, npyPath):
    """Loading weights from npy file
    
    #Arguments
        model: The model needs to load weights.
        npyPath: Path to numpy npy weights file.
    """
...
def saveWeightsToNumpy(model, npyPath):
    """Saving weights to npy file
    
    #Arguments
        model: The model needs to save weights.
        npyPath: Path to numpy npy weights file.
    """
    params = {}
    key_list = ['fc6', 'fc7', 'conv7_2', 'conv7_1', 'conv8_2_mbox_conf', 'conv6_2', 'conv1_2', 'conv7_2_mbox_conf', 'conv6_1', 'conv5_1', 'conv5_3', 'conv5_2', 'conv4_1', 'conv4_2', 'conv4_3', 'conv3_3', 'conv3_2', 'conv3_1', 'conv4_3_norm', 'conv4_3_norm_mbox_conf', 'conv2_2', 'conv2_1', 'conv6_2_mbox_loc', 'fc7_mbox_conf', 'conv8_1', 'conv8_2', 'conv6_2_mbox_conf', 'pool6_mbox_conf', 'fc7_mbox_loc', 'conv4_3_norm_mbox_loc', 'conv7_2_mbox_loc', 'conv1_1', 'conv8_2_mbox_loc', 'pool6_mbox_loc']
    for L in model.layers:
        if L.name in key_list:
            v = {}
            param = L.get_weights()
            if len(param) == 2:
...
def loadGT(gtPath, imgPath):
    """Load ground truth from icdar test set
    #Arguments
        gtPath: Path to the ground truth folder
        imgPath: Path to the test image file folder
        
    #Return
        The numpy array has [xmin, ymin, xmax, ymax, id(one hot)...]
        for all pictures.
